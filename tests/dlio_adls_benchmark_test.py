...
import logging
import os
from dlio_benchmark.utils.config import ConfigArguments, GetConfig, LoadConfig
...
@pytest.mark.timeout(TEST_TIMEOUT_SECONDS, method="thread")
def test_adls_rejects_unsupported_format(setup_test_env) -> None:
    storage_root, storage_type, mock_file_system_client, adls_overrides = setup_test_env
    ...
    with patch("dlio_benchmark.storage.adls_gen2_storage.DataLakeServiceClient") as mock_service, \
        ...
        with initialize_config_dir(version_base=None, config_dir=config_dir):
            ...
            with pytest.raises(
                Exception,
                match="For ADLS Gen2 using PyTorch framework, only NPZ or NPY formats are supported",
            ):
                DLIOBenchmark(cfg['workload'])
...
def test_adls_checkpoint_duration_options_are_configurable():
    from dlio_benchmark.checkpointing.pytorch_adls_checkpointing import PyTorchADLSCheckpointing

    checkpoint = PyTorchADLSCheckpointing.__new__(PyTorchADLSCheckpointing)

    assert checkpoint._get_duration_option({}, "container_sas_ttl", timedelta(hours=1)) == timedelta(hours=1)
    assert checkpoint._get_duration_option({"container_sas_ttl": 90}, "container_sas_ttl", timedelta(hours=1)) == timedelta(seconds=90)
    assert checkpoint._get_duration_option({"container_sas_ttl": "2h"}, "container_sas_ttl", timedelta(hours=1)) == timedelta(hours=2)
    assert checkpoint._get_duration_option({"sas_refresh_margin": "7.5m"}, "sas_refresh_margin", timedelta(minutes=5)) == timedelta(minutes=7.5)


def test_adls_get_config_reads_storage_options():
    ConfigArguments.reset()
    args = ConfigArguments.get_instance()
    ...
    assert GetConfig(args, "storage.storage_options.account_name") == "testacct"
    assert GetConfig(args, "storage.storage_options.container_sas_ttl") == "1:00:00"
    assert GetConfig(args, "storage.storage_options.sas_refresh_margin") == "0:05:00"


def test_adls_checkpoint_uses_none_blobio_credential(monkeypatch):
    from dlio_benchmark.checkpointing import pytorch_adls_checkpointing as module
    from dlio_benchmark.checkpointing.pytorch_adls_checkpointing import PyTorchADLSCheckpointing
    ...
    class MockBlobIO:
        def __init__(self, blob_url, mode, credential=None):
            blobio_calls.append({"blob_url": blob_url, "mode": mode, "credential": credential})
    ...
    monkeypatch.setattr(module, "BlobIO", MockBlobIO)
    monkeypatch.setattr(module.torch, "save", lambda state, writer: None)

    checkpoint = PyTorchADLSCheckpointing.__new__(PyTorchADLSCheckpointing)
    checkpoint._shared_access_signature = None
    checkpoint._account_key = None
    checkpoint.get_name = lambda suffix: "https://testacct.blob.core.windows.net/cont/checkpoints/model.pt?sig=abc"

    checkpoint.save_state("model.pt", {"step": 1})

    assert len(blobio_calls) == 1
    assert blobio_calls[0]["credential"] is None
