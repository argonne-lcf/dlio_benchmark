...
##COBALT -n 128 -A datascience -t 3:00:00 -q default --jobname=dlio_cosmic_overlap --attrs mcdram=cache:numa=quad
...
for i in 0 0.00001 0.00002 0.00003 0.00004; do
  n=$COBALT_JOBSIZE
  computation_time=${i}
  if [ "${i}" = "0" ]; then
    data_type=hdf5
    OPTS=(-f ${data_type} -fa shared -ct ${computation_time} -nf 1 -sf 3072000 -rl 40960 -bs 1 -df ${APP_DATA_DIR} -gd 0 -k 1 -p 1 -l /projects/datascience/dhari/tf_logs/cosmic_overlap_${i}_n_${n})
  else
    data_type=hdf5_opt
    OPTS=(-f ${data_type} -fa shared -ct ${computation_time} -nf 1 -sf 3072000 -rl 40960 -bs 1 -df ${APP_DATA_DIR} -ts 1048576 -rp 1 -ps 3072000 -gd 0 -k 1 -p 1 -l /projects/datascience/dhari/tf_logs/cosmic_overlap_${i}_n_${n})
  fi
