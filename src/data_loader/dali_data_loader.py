...
class DaliDataLoader(BaseDataLoader):
    ...
    def read(self, epoch_number):
        ...
        for thread_index in range(num_threads):
            dataset = DaliDataset(self.format_type, self.dataset_type, epoch_number, num_samples, batch_size, thread_index)
        # None executes pipeline on CPU and the reader does the batching
            pipeline = Pipeline(batch_size=batch_size, num_threads=1, device_id=None)
            with pipeline:
        ...
        self._dataset = DALIGenericIterator(pipelines, ['data', 'label'], size=num_samples)
