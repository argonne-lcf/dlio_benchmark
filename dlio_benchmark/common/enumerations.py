...
class FormatType(Enum):
  ...
  def get_enum(value):
    ...
    elif FormatType.DCM.value == value:
      return FormatType.DCM
...
class ReadType(Enum):
  ...
  - On Demand: loading data in a batch-by-batch fashion - In Memory: loading
  data all at once in the beginning.
  """
...
class FileAccess(Enum):
  ...
  - Multi = save dataset into multiple files - Shared = save everything in a
  single file - Collective = specific for the shared case, when we want to do
  collective I/O. Typically used for a huge file with small objects.

    One thread T reads from disk and the other threads read from T's memory,
